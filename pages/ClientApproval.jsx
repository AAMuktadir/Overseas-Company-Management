...
import { updateClientApproval } from "../libs/pocketbase";
...
export default function ClientApproval() {
  ...
  const EditClientModalHandler = async (client) => {
    ...
    console.log(client);
    const result = await updateClientApproval(client);
    if (result) {
      getClientHandler();
    }
  };
  ...
  return (
    <>
      ...
      <main>
        <Header title={"Clients Validity"}></Header>
        <div className="p-5">
          <h1>Here we can see Validity of Clients</h1>
        </div>
        <div className="p-5">
          <Table
            aria-label="Clients"
          ...
          >
            <Table.Header>
              ...
              <Table.Column>CLIENT VALIDITY</Table.Column>
              ...
              {Clients &&
                Clients.map((client) => (
                  <Table.Row key={client.id}>
                    ...
                    <Table.Cell>
                      {getIsValid(client.isreject) == "Approved" ? (
                    ...
                    <Table.Cell>
                      {getIsValid(client.isreject) == "Approved" ? null : (
